...
(function () {
    ...
    async function rotateStatus(el) {
        ...
        try {
            ...
            setTimeout(() => el.classList.remove('is-flashing'), 400);
        } catch (err) {
    ...
    editForm.addEventListener('submit', async (e) => {
        ...
        if (countDiff < 0) {
            ...
            if (busy.length > 0) {
                ...
                showBlocked(`No se puede reducir porque ${nums} no está${busy.length > 1 ? 'n' : ''} disponible${busy.length > 1 ? 's' : ''}.`);
                return;
            }
        }
        ...
        if (countDiff !== 0) {
            ...
            if (countDiff > 0) {
                ...
                msg = `Vas a eliminar ${Math.abs(countDiff)} ${Math.abs(countDiff) === 1 ? 'mesa' : 'mesas'}. ¿Confirmar?`;
            }
    ...
    createForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            submitCreate.textContent = 'Crear';
        }
    });
